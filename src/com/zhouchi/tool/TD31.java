...
public class TD31 {
    public static void main(String[] args) throws Exception {
        ...
        //读取需统计用户的数据
        InputStream streamUser = new FileInputStream("E:\\Daily\\TD31.xlsx");
        if (streamUser == null) {
            streamWriterInfo.append("TD31全网通话记录<TD31.xlsx>不存在或文件命名错误，请核实！！！\n");
            return;
        }
        ...
        //读取FD NH方向用户的数据
        InputStream streamUserNH = new FileInputStream("E:\\Daily\\TDNH31.xlsx");
        if (streamUserNH == null) {
            streamWriterInfo.append("TD31 NH方向用户名单<TDNH31.xlsx>不存在或文件命名错误，请核实！！！\n");
            return;
        }
        ...
        //读取FD NH方向用户的数据
        InputStream streamUserTH = new FileInputStream("E:\\Daily\\TDTH31.xlsx");
        if (streamUserTH == null) {
            streamWriterInfo.append("TD31 TH方向用户名单<TDTH31.xlsx>不存在或文件命名错误，请核实！！！\n");
            return;
        }
        ...
        File fileAll = new File("E:\\Daily\\TD31.txt");
        File fileTH = new File("E:\\Daily\\TDTH31.txt");
        File fileNH = new File("E:\\Daily\\TDNH31.txt");
        ...
        //获取所有通信用户站列表;
        String path = "E:\\Daily\\TD31.xlsx";

        calAllRate(path, userAll,fileAll,"TD31全网呼通率统计");
        calIRate(path, userAll, userNH, userNHResult, fileNH,"TD31 NH方向呼通率统计");
        calIRate(path, userAll, userTH, userTHResult, fileTH,"TD31 TH方向呼通率统计");
    }
                    ...
                    if (callAddress == null) {
                        ...
                        if (user.contains(callAddress.toString())) {
                            if (("成功".equals(result.toString()))) {
                                ...
                                System.out.println("31 " + callAddress.toString());
                                successCount++;
                            }
                        }
                    }
